...
function scan() {
  const found = [...document.images]
    ...
    .filter((image) => {
      ...
      try {
        const protocol = new URL(image.currentSrc || image.src, location.href).protocol
  ...
  candidates = found.map((image, index) => {
    const rect = image.getBoundingClientRect()
    const sourceUrl = new URL(image.currentSrc || image.src, location.href).href
    return {
      ...
      intrinsicWidth: image.naturalWidth || Math.round(rect.width), intrinsicHeight: image.naturalHeight || Math.round(rect.height),
    }
  })
...
function renderOverlay(item, region, page) {
  ...
  const pageWidth = Number(page?.width) || image.naturalWidth || image.clientWidth
  const pageHeight = Number(page?.height) || image.naturalHeight || image.clientHeight
  overlay.style.left = `${image.offsetLeft + (Number(region.x) / pageWidth) * image.clientWidth}px`
  overlay.style.top = `${image.offsetTop + (Number(region.y) / pageHeight) * image.clientHeight}px`
  overlay.style.width = `${Math.max(12, (Number(region.width) / pageWidth) * image.clientWidth)}px`
  overlay.style.minHeight = `${Math.max(24, (Number(region.height) / pageHeight) * image.clientHeight)}px`
