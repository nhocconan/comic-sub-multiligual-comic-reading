...
test('reader scans AMP comic image hosts used by Baozimh', () => {
  ...
  assert.match(source, /function beginViewportLock\(\)/)
  assert.match(source, /window\.addEventListener\('scroll', restoreLockedViewport, \{ capture: true \}\)/)
  assert.match(source, /window\.scrollTo\(\{ left: x, top: y, behavior: 'auto' \}\)/)
  assert.match(source, /function endViewportLock\(\)/)
  assert.match(source, /window\.removeEventListener\('scroll', restoreLockedViewport, \{ capture: true \}\)/)
  assert.match(source, /function startAll\(\) \{\n  beginViewportLock\(\)/)
  assert.match(source, /window\.addEventListener\('beforeunload', endViewportLock\)/)
