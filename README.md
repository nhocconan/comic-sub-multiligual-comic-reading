...
Comic Sub translates text on comic images while you read them on their original
website. Click the extension on the current tab, grant access only to the image
origins that page uses, and keep scrolling. Vietnamese translations appear in
bounded opaque boxes over the detected dialogue. The original image element is
never replaced.
...
Compose elsewhere. `native` and `docker` force either mode. Both expose:
...
Double-click `stop.command` to stop either backend. Docker model and credential
...
1. Open any chapter page containing ordinary browser images.
2. Click **Comic Sub → Dịch trang này**.
3. Review the candidate count and grant the exact image-origin permissions.
4. Keep scrolling. The visible page and configured look-ahead are translated.
5. Use **Reveal source**, **Pause**, **Exclude**, or **Retry** at any time.
