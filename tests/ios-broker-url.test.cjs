...
test('CJK on-device OCR refines suspicious fast results and rejects Latin garbage', () => {
  ...
  assert.match(source, /let tiles = overlappingTiles\(cgImage\)/)
  assert.match(source, /async let tile0 = recognizePass\(/)
  assert.match(source, /async let tile3 = recognizePass\(/)
  assert.match(source, /mergeRecognitionPasses\(/)
  assert.match(source, /candidateCoverage >= 0\.34/)
  assert.match(source, /candidateIsMoreComplete/)
  assert.match(source, /request\.customWords = \[/)
  ...
  assert.match(source, /const fitText = \(node, box, bounds\) =>/)
  assert.doesNotMatch(source, /box\.width \* 1\.65/)
  assert.match(source, /const maxWidthRatio = targetLength > 60 \? \.46/)
  assert.match(source, /const maxHeightRatio = targetLength > 60 \? \.24/)
  assert.match(source, /const baseBoxes = displayRegions\.map/)
  assert.match(source, /if \(overlapRatio >= \.08\)/)
  assert.match(source, /while \(size > 6\.5/)
  assert.match(source, /if \(!semanticOnly\) fitText\(node, boxes\[index\], target\.rect\)/)
